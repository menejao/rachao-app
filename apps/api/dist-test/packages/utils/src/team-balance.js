...
exports.generateBalancedTeams = generateBalancedTeams;
const TEAM_NAMES = [
    "Time Azul",
    "Time Laranja",
    "Time Verde",
...
const POSITION_PRIORITY = ["GOLEIRO", "FIXO", "ALA", "PIVO", "CORINGA"];
function inferTeamCount(playerCount) {
    if (playerCount >= 18)
        return 3;
    if (playerCount >= 28)
        return 4;
...
function countPosition(team, posicao) {
    return team.jogadores.filter((item) => item.posicao === posicao).length;
}
function pickBestTeam(teams, player) {
...
function generateBalancedTeams(players, requestedTeamCount) {
    if (players.length < 2) {
    ...
    const byPriority = POSITION_PRIORITY.flatMap((posicao) => players
