...
export interface DemoGame {
  ...
  turmaId: string;
  dataJogo: string;
  status: "CONFIRMACAO_ABERTA" | "TIMES_GERADOS" | "RASCUNHO" | "FECHADO" | "FINALIZADO";
}

export interface DemoTeam extends GeneratedTeam {
  id: string;
  jogoId: string;
}

export interface DemoGoal extends GoalSummary {}

export const demoStore: {
  turmas: Array<TurmaSummary & { whatsappGroupId?: string; whatsappProvider?: WhatsAppProvider }>;
...
export function addDemoLog(entry: Omit<EventoLogEntry, "id" | "createdAt">) {
  demoStore.logs.unshift({
    id: `log-${demoStore.logs.length + 1}`,
    createdAt: new Date().toISOString(),
    ...entry,
  });
}

export function mapConfirmedPlayersForGame(jogoId: string): TeamDraftPlayer[] {
  return demoStore.presencas
    .filter((presence) => presence.jogoId === jogoId && presence.resposta === "SIM")
    .map((presence) => demoStore.jogadores.find((player) => player.id === presence.jogadorId)!)
    .filter(Boolean)
    .map((player) => ({
      id: player.id,
      nome: player.nome,
...
export function mapTimeSummaries(): TimeSummary[] {
  return demoStore.times.map((team) => {
    const jogo = demoStore.jogos.find((item) => item.id === team.jogoId);
