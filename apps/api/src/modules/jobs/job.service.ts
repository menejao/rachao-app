...
export class JobService {
  ...
  async run(job: WeeklyJobName, referenceDate = new Date()) {
    const startedAt = new Date().toISOString();

    try {
      let result:
        | { processed: number; jogos?: string[]; turmas?: string[] }
        | { processed: number; jogos: string[]; turmas?: string[] };
  ...
  private async sendConfirmation(referenceDate: Date) {
  ...
  private async remindPending() {
    const reminders = await this.repository.listPendingReminders();

    for (const item of reminders) {
      const sender = makeWhatsAppSender(item.provider);
  ...
  private async closeList() {
    ...
    for (const jogo of jogos) {
      ...
      const sender = jogo.groupId ? makeWhatsAppSender(jogo.provider) : null;
      if (sender && jogo.groupId) {
        await sender.sendGroupMessage({
          provider: jogo.provider,
          groupId: jogo.groupId,
          message: buildListClosedMessage({
      ...
      try {
        ...
        if (sender && jogo.groupId) {
          await sender.sendGroupMessage({
            provider: jogo.provider,
            groupId: jogo.groupId,
            message: buildTeamsGeneratedMessage({
              turmaNome: jogo.turmaNome,
              teams,
            }),
          });
...
function getNextOccurrence(referenceDate: Date, targetDayOfWeek: number, horario: string) {
