...
export function GeralForm({ turma }: { turma: TurmaSummary }) {
  ...
  async function handleSave() {
    ...
    if (!/^\d{2}:\d{2}$/.test(form.horario)) { setError("Horário inválido. Use HH:MM."); return; }
    setSaving(true);
    setError(null);
    try {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          >
            {DIAS.map((d, i) => (
              <option key={i} value={i} className="bg-[#0a1628]">{d}</option>
            ))}
          </select>
        </div>
        <div>
      ...
      <div>
        ...
        <select
          value={form.status}
          onChange={(e) => field("status", e.target.value as "ATIVA" | "INATIVA")}
          className={inputClass}
        >
          <option value="ATIVA" className="bg-[#0a1628]">Ativa</option>
          <option value="INATIVA" className="bg-[#0a1628]">Inativa</option>
        </select>
      </div>
